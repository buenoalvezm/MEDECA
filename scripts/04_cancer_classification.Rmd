...
selected_proteins <- 
  step_filter_auc(data = medeca_train,
                  cutoff = 0.65)
# Select proteins
medeca_train_filtered <- 
  medeca_train |> 
  select(Sample, Cancer, selected_proteins$final_list$Protein)

# Explore correlation filter
MAD1L1, DPY30 
...
multiclass_train <- 
  ...
  select(-Cancer) |> 
  left_join(meta_medeca |> 
              select(Sample, Category), by = "Sample") 
...
# Run ML function
medeca_multiclass <- 
  do_lasso_multiclass(variable = "Category",
                    split_train = multiclass_train, 
                    split_test = multiclass_test) 
...
# medeca_multiclass$final_fit  |> 
#   vi()
#   extract_fit_parsnip() %>%
#   vi(lambda = medeca_multiclass$best_glmnet$penalty) %>%
#   mutate(
...
plot_lollipop(cancer_all) 

ggsave(savepath("lollipop_multiclass_cancer.pdf"), h = 10, w = 3)
```
...
proteins_cancer_de <- 
  important_proteins |> 
  filter(class == "Cancer") |> 
  left_join(medeca_de, by = c("term" = "Assay")) |> 
  filter(sig != "not significant")

plot_beeswarm_cohorts(proteins = proteins_cancer_de$term,
                      data = data,
                      metadata = meta_medeca |> 
                        mutate(Cohort = "MEDECA",
                               Disease_type = factor(Disease_type, levels = names(pal_disease_type))),
                      variable = "Disease_type",
                      palette = pal_disease_type,
                      angled = T)

ggsave(savepath("proteins_cancer_multiclass.pdf"), h = 9, w = 4)
...
allvos_multiclass_fit <- predict(medeca_multiclass$final_fit|> extract_workflow(), 
                      ml_data_allvos, 
                      type = "prob")

allvos_multiclass_fit |>
  mutate(Class = ml_data_allvos$Category,
         Sample = ml_data_allvos$Sample) |> 
  pivot_longer(names_to = ".pred_class",
               values_to = "probability",
               cols = c(1:3)) |> 
  mutate(.pred_class = gsub(".pred_", "", .pred_class)) |> 
  group_by(Sample) |> 
  top_n(1, probability) |> 
