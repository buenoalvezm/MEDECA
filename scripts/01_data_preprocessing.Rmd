...
pca_data_panels <- 
  map_df(panels, function(panel) {
    ...
    pca_dat <- 
      ...
      impute_values(wide_data = T, ID = "SampleID") |> 
...
pca_data_panels |> 
  ...
  scale_color_manual(values = pal_batch) +
  facet_wrap(~Panel) +
  theme_hpa()
...
pca_scores_panel <-
  ...
  ggplot(aes(PC1, PC2)) +
  geom_point() +
  geom_text_repel(data = labels, aes(label = SampleID), show.legend = F) +
  theme_hpa()
...
pca_loadings <- 
  pca_data_loadings |> 
  left_join(raw_data |> 
              distinct(Assay, Panel), by = c("Protein" = "Assay")) |> 
  ggplot(aes(PC1, PC2, color = Panel)) +
  geom_point() +
  geom_text_repel(aes(label = Protein), show.legend = F) +
...
data_qc |> 
  ...
  summarise(Median = median(NPX),
            IQR = IQR(NPX)) |> 
  ggplot(aes(Median, IQR)) +
  geom_point() +
  geom_text_repel(aes(label = Assay), size = 3) +
  facet_wrap(~Panel, scales = "free") +
  theme_hpa()

ggsave(savepath("median_iqr.png"), h = 8, w = 8)
```
